...
const film = document.querySelector(".film__section");
const technology = document.querySelector(".technology__section");
window.onscroll = () => {
  const topCtaSection = cta.getBoundingClientRect().top;
  const bottomCtaSection = cta.getBoundingClientRect().bottom;
  const topFilmSection = film.getBoundingClientRect().top;
  const bottomFilmSection = film.getBoundingClientRect().bottom;
  const topTechnologySection = technology.getBoundingClientRect().top;
  const bottomTechnologySection = technology.getBoundingClientRect().bottom;
  contactLinks.forEach((contact) => {
    const topContactLink = contact.getBoundingClientRect().top;
    if (
      (topCtaSection < topContactLink + 10 &&
        bottomCtaSection > topContactLink) ||
      (topTechnologySection < topContactLink + 10 &&
        bottomTechnologySection > topContactLink) ||
      (topFilmSection < topContactLink + 10 &&
        bottomFilmSection > topContactLink)
    ) {
...
/* ====================================================
   Changing The Cursor When Hovering Over The Technology Items
  ===================================================== */
const technologyWrapper = document.querySelector(".splide__technology");
technologyWrapper.addEventListener("mousemove", (e) => {
  const bound = technologyWrapper.getBoundingClientRect();
  const mouseY = (e.clientY - bound.top).toFixed();
  const mouseX = (e.clientX - bound.left).toFixed();
  const cursorCustome = technologyWrapper.querySelector(".splide__cursor");
  cursorCustome.classList.add("active");
  document.body.style.cursor = "none";
  cursorCustome.style.top = `${mouseY}px`;
  cursorCustome.style.left = `${mouseX}px`;
});
technologyWrapper.addEventListener("mouseout", () => {
  const cursorCustome = technologyWrapper.querySelector(".splide__cursor");
