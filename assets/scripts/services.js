...
const services = document.querySelector(".services__section");
const ctaTwo = document.querySelector(".cta__two_section");
window.onscroll = () => {
  const topServicesSection = services.getBoundingClientRect().top;
  const bottomServicesSection = services.getBoundingClientRect().bottom;
  ...
  contactLinks.forEach((contact) => {
    ...
    if (
      (topServicesSection < topContactLink + 10 &&
        bottomServicesSection > topContactLink) ||
...
const servicesNode = document.querySelectorAll(".service__node");
...
servicesBtn.forEach((btn) => {
  btn.addEventListener("click", () => {
    ...
    disableAllServices();
    document
      .querySelector(`.services__items #${targetId}`)
      .classList.add("active");
  });
});
servicesNode.forEach((node) => {
