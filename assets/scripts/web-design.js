...
const typesWeb = document.querySelector(".types__web_section");
const cta = document.querySelector(".cta__section");
const quote = document.querySelector(".quote__section");
window.onscroll = () => {
  const topTypesWebSection = typesWeb.getBoundingClientRect().top;
  const bottomTypesWebSection = typesWeb.getBoundingClientRect().bottom;
  const topCtaSection = cta.getBoundingClientRect().top;
  const bottomCtaSection = cta.getBoundingClientRect().bottom;
  const topQuoteSection = quote.getBoundingClientRect().top;
  const bottomQuoteSection = quote.getBoundingClientRect().bottom;
  contactLinks.forEach((contact) => {
    const topContactLink = contact.getBoundingClientRect().top;
    if (
      (topTypesWebSection < topContactLink + 10 &&
        bottomTypesWebSection > topContactLink) ||
      (topCtaSection < topContactLink + 10 &&
        bottomCtaSection > topContactLink) ||
      (topQuoteSection < topContactLink + 10 &&
        bottomQuoteSection > topContactLink)
    ) {
...
new Splide(".splide__types_design", {
  pagination: false,
  arrows: false,
  perPage: 1,
  gap: "50px",
...
/* ====================================================
   Change Web Type Items 
  ===================================================== */
const typeAppBtns = document.querySelectorAll(".type__web_btn");
...
// Related projects wrapper cursor
